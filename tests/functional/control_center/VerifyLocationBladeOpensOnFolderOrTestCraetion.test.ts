...
*/
test('@Smoke @SyntheticControlCenter Verify When user selects Folder on create blade then location selection blade should display', async({sideNavigationBar, syntheticFolderDetailPage,synLocationBlade,synCCPage, verification}) => {
    
    await sideNavigationBar.navigateToSyntheticCCFromSideNavigation();
    await syntheticFolderDetailPage.goToNewFolderCreate();
    await synCCPage.delay(3000);
